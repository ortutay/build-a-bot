...
type CodePromptOptions = PromptOptions & {
  reports: string;
...
const setup = (options: PromptOptions): string => `Here is the user input:
<user-input>
URL: ${options.url}
...
export const access = (
  options: PromptOptions
): string => `You are part of a team of agents building an API based on a URL and user prompt using JavaScript. Your specific task is to determine the best way surface data on this website. You need to figure out cost/speed/data coverage optimizations, as well as a nice input/output schema.
...
export const plan = (
  options: PromptOptions
): string => `You are writing a JavaScript web scraping script. Explore and gather information necessary to write this script.
...
): string => `You are writing a JavaScript web scraping script. You have various reports from sub-agents. Use these to write reports.
