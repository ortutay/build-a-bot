...
CREATE UNIQUE INDEX `scripts_active_data_service_id_name_unique` ON `scripts` (`data_service_id`,`name`) WHERE `active` = 1;--> statement-breakpoint
...
CREATE TABLE `items` (
	...
	CONSTRAINT `items_source_script_id_source_url_unique_id_unique` UNIQUE(`source_script_id`,`source_url`,`unique_id`)
);--> statement-breakpoint
