...
const createRunJsSnippetTool = (documentLibrary: DocumentLibrary): any => {
  const snippetContext = createSnippetContext(documentLibrary);
  return createTool({
...
`,
    inputSchema: z.object({
      ...
      code: z
        .string()
        .describe('JavaScript code snippet to run. It will be a run in a VM context.'),
    }),
...
export const createTools = (options: CreateToolsOptions): Record<string, Tool> => {
  const internal = [createRunJsSnippetTool(options.documentLibrary)];
  return Object.fromEntries(internal.map((tool) => [tool.id, tool]));
};

export const tools = createTools({ documentLibrary: defaultDocumentLibrary });
